...
export async function sendVerificationEmail(
  recipient: LoginResponse
): Promise<string> {
  const verificationKey = signEmailVerification(recipient);

  const link = `http://${process.env.HOST}:${process.env.SERVER_PORT}/api/?key=${verificationKey}`;

  const message = {
    to: recipient.email,
    from: "attila.huszar@outlook.com",
    subject: "Email verification - Fox Ticket",
    text: `${recipient.name}, please verify your email by copying this link to the browser address bar: ${link}`,
    html: `
    <div style="font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", "Roboto", "Oxygen", "Ubuntu", "Cantarell", "Fira Sans", "Droid Sans", "Helvetica Neue", sans-serif;">
      ...
      <h2 style="text-align: center;">${recipient.name}, please verify your email address by clicking <b><a href=${link}>this link</a></b>.</h2>
